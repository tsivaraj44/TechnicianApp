...
//        drawVerticalDividers(canvas, parent)
        ...
        val paint = Paint()
        paint.setColor(ContextCompat.getColor(parent.context, R.color.black))
    ...
    private fun drawHorizontalDividers(canvas: Canvas, parent: RecyclerView) {
        ...
        for (i in 1 until mNumColumns) {
            var lastRowChildIndex: Int
            lastRowChildIndex = if (i < lastRowChildCount) {
            ...
            val dividerTop: Int = firstRowChild.getTop()
            val dividerRight: Int = firstRowChild.getLeft()
            val dividerLeft = dividerRight - mHorizontalDivider?.intrinsicWidth!!
            val dividerBottom: Int = lastRowChild.getBottom()
    ...
    private fun drawVerticalDividers(canvas: Canvas, parent: RecyclerView) {
        ...
        for (i in 1..rowCount) {
            ...
            val leftmostChild: View = parent.getChildAt(i * mNumColumns)
            val rightmostChild: View = parent.getChildAt(rightmostChildIndex)
            val dividerLeft: Int = leftmostChild.getLeft()
            val dividerBottom: Int = leftmostChild.getTop()
            val dividerTop = dividerBottom - mVerticalDivider?.intrinsicHeight!!
            val dividerRight: Int = rightmostChild.getRight()
